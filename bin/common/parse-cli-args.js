...
const CONFIG_OPTION = /^--([^=]+?)(?:=(.+))$/
const PACKAGE_CONFIG_PATTERN = /^npx_package_config_(.+)$/
...
function createPackageConfig() {
    const retv = {}
    const packageName = process.env.npx_package_name
    if (!packageName) {
        return retv
    }
...
class ArgumentSet {
    ...
    constructor(initialValues, options) {
        ...
        this.race = false
        this.rest = []
        this.silent = process.env.npx_config_loglevel === "silent"
